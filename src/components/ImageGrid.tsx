      ...
      {results.map((result) => (
        ...
        >
          {result.imagePath ? (
            ...
            <div className="flex h-64 w-full items-center justify-center bg-zinc-800 text-xs text-zinc-600">{result.primaryText}</div>
          )}
          <div className="p-2">
            <p className="truncate text-xs font-bold text-white">{result.primaryText}</p>
            {result.secondaryText && <p className="truncate text-xs text-zinc-500">{result.secondaryText}</p>}
          </div>
        </div>
      ))}
    </div>
